...
int main (int argc, char *argv[]) {
   ...
   //fflush (stdout);
   remaindPart = numprocs;

   for(; remaindPart!=1;){
       int maxElement = remaindPart;
       remaindPart = (remaindPart&1) + (remaindPart>>1);
       if(id<remaindPart){
           if(id+remaindPart < maxElement){
               MPI_Status status;
               int partnerCount = 0;
               MPI_Recv(&partnerCount, 1, MPI_INTEGER, id+remaindPart,0,MPI_COMM_WORLD,&status);
               localCount += partnerCount;
           }
       }else if(id<maxElement){
           MPI_Send(&localCount, 1, MPI_INTEGER, id-remaindPart, 0, MPI_COMM_WORLD);
       }
   }
