...
void  * smooth_function(void * rank);
long  thread_count;     //total thread count


pthread_mutex_t barrier_mutex = PTHREAD_MUTEX_INITIALIZER;   //barrier
...
/*********************************************************/
        ...
        for(long i=0; i<thread_count; i++){
                pthread_create(&thread_handles[i], NULL, smooth_function, (void *) (uintptr_t)i);
	}
...
void  * smooth_function(void * rank){
	long local_rank = (long) rank;
        int size = bmpInfo.biHeight / thread_count;
        int extra = bmpInfo.biHeight % thread_count;
        int shift_base = (local_rank < extra ?  (local_rank * (++size) ):( (size * local_rank) + extra));
	...
	for (int i = shift_base; i < shift_base + size; i++){
